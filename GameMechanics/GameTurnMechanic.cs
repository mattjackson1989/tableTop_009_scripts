...
public class GameTurnMechanic : MonoBehaviour
{
    ...
    public static List<GameObject> InitiativeList;
    ...
    void Update()
    {
        ...
        {
            PlayerGameController.CheckIfEscapeIsPressed();
            PlayerGameController.CheckCurrentLocation();
            PlayerGameController.CheckKeyboardInputMovement();
            ProcessTurn();
        }
    ...
    private void ProcessTurn()
    {
        ...
        if (InitiativeList[CurrentTurnNumber].GetComponent<GenericCreature>().TotalActionPointsLeft <= 0)
        {
            ...
            if (CurrentTurnNumber + 1 >= TurnNumberLimit)
            {
                CurrentTurnNumber = 0;
                Round++;
                TurnManager.Round = Round;
                
                InitiativeList[CurrentTurnNumber].GetComponent<GenericCreature>().IsTurnActive = true;
                
                TurnManager.CurrentCreaturesTurn = InitiativeList[CurrentTurnNumber].GetComponent<GenericCreature>();
                TurnManager.CurrentCreaturesTurnIndex = CurrentTurnNumber;
                TurnManager.CurrentListOfCreatures = InitiativeList;
                ...
                PlayerUIController.CurrentPlayersName = InitiativeList[CurrentTurnNumber].GetComponent<GenericCreature>().name;
                PlayerUIController.ActionPointsLeftOnTurn = InitiativeList[CurrentTurnNumber].GetComponent<GenericCreature>().TotalActionPointsLeft;
                PlayerUIController.MovementLeftOnTurn = InitiativeList[CurrentTurnNumber].GetComponent<GenericCreature>().TotalSpeedLeft;
